...
struct Vector3
{
	...
	// ctors
	Vector3()restrict(amp) : x(0), y(0), z(0) {};
	Vector3(float x, float y, float z) restrict(amp) : x(x), y(y), z(z) {};
	...
	float       distance(const Vector3& vec) const restrict(amp);     // distance between two vectors
	Vector3&    normalize();                            //
	void normalizeAMP() restrict(amp);
	float       dot(const Vector3& vec) const;          // dot product
	float       dot(const Vector3& vec) const restrict(amp);          // dot product
	Vector3     cross(const Vector3& vec) const;        // cross product
	Vector3     cross(const Vector3& vec) const restrict(amp);        // cross product
	bool        equal(const Vector3& vec, float e) const; // compare with epsilon

														  // operators
	Vector3     operator-() const;                      // unary operator (negate)
	Vector3     operator+(const Vector3& rhs) const;    // add rhs
	Vector3     operator+(const Vector3& rhs) const restrict(amp);    // add rhs
	Vector3     operator-(const Vector3& rhs) const;    // subtract rhs
	Vector3     operator-(const Vector3& rhs) const restrict(amp);;    // subtract rhs
	Vector3&    operator+=(const Vector3& rhs);         // add rhs and update this object
	Vector3&    operator-=(const Vector3& rhs);         // subtract rhs and update this object
	Vector3     operator*(const float scale) const;     // scale
	Vector3     operator*(const float scale) const restrict(amp);     // scale
	...
	Vector3     operator/(const float scale) const restrict(amp);     // inverse scale
	Vector3&    operator/=(const float scale);          // scale and update this object
...
inline Vector3 Vector3::operator+(const Vector3& rhs) const restrict(amp) {
	return Vector3(x + rhs.x, y + rhs.y, z + rhs.z);
}
...
inline Vector3 Vector3::operator*(const float a) const  restrict(amp) {
	return Vector3(x*a, y*a, z*a);
}
...
inline float Vector3::length() const  restrict(amp) {
	return fast_math::sqrtf(x*x + y*y + z*z);
}
...
inline float Vector3::distance(const Vector3& vec) const restrict(amp) {
	return fast_math::sqrtf((vec.x - x)*(vec.x - x) + (vec.y - y)*(vec.y - y) + (vec.z - z)*(vec.z - z));
}
...
inline void Vector3::normalizeAMP() restrict(amp)
{
	float xxyyzz = x*x + y*y + z*z;
	//@@if(xxyyzz < EPSILON)
	//@@    return *this; // do nothing if it is ~zero vector
	...
	//float invLength = invSqrt(xxyyzz);
	float invLength = 1.0f / fast_math::sqrtf(xxyyzz);
	x *= invLength;
	y *= invLength;
	z *= invLength;
}
...
inline float Vector3::dot(const Vector3& rhs) const   restrict(amp) {
	return (x*rhs.x + y*rhs.y + z*rhs.z);
}
...
inline Vector3 Vector3::cross(const Vector3& rhs) const restrict(amp)
{
	return Vector3(y*rhs.z - z*rhs.y, z*rhs.x - x*rhs.z, x*rhs.y - y*rhs.x);
}
